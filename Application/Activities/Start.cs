...
namespace WordLearning.Application.Activities
{
    ...
    public class Start : CustomActivity
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            WlFolderAdapter adapter;

            if (savedInstanceState == null)
            {
                var folder = 
                adapter = new WlFolderAdapter(this, WlUtility.RootFolder);
            }
            else
            {
                WlFolder currentFolder = savedInstanceState.GetExtra<WlFolder>(CURRENT_FOLDER_KEY);
                adapter = new WlFolderAdapter(this, currentFolder);
            }
            ...
            _disposables.Add(adapter.SubscribeFolderChangedObservable(value =>
            {
                ...
                disposable = value.RegisterPropertyValueChanged(nameof(WlFolder.Name), handler: name => SupportActionBar.Title = name.ToString());
            }));
        ...
        public override bool OnCreateOptionsMenu(IMenu menu)
        {
            bool isVisible = ToolbarState is StartDeleteToolbarState
                || _listView.GetAdapter<WlFolderAdapter>().CurrentFolder != WlUtility.RootFolder;
        ...
        protected override void OnSaveInstanceState(Bundle outState)
        {
            ...
            outState.PutExtra(CURRENT_FOLDER_KEY, _listView.GetAdapter<WlFolderAdapter>().CurrentFolder);
        }
        ...
        public override bool OnOptionsItemSelected(IMenuItem item)
        {
            switch (item.ItemId)
            {
                case Resource.Id.action_add_Start_Init:
                    ...
                    {
                        WlFolder targetFolder = _listView.GetAdapter<WlFolderAdapter>().CurrentFolder;
                ...
                case Android.Resource.Id.Home:
                    switch (ToolbarState.ToolbarId)
                    {
                        case Resource.Id.tbStart_Init:
                            WlFolderAdapter adapter = _listView.GetAdapter<WlFolderAdapter>();
                            adapter.CurrentFolder = adapter.CurrentFolder.GetParent<WlFolder>();
                            break;
                        case Resource.Id.tbStart_Deletemode:
                            _listView.GetAdapter<WlFolderAdapter>().CurrentFolder.ClearCheck();
                ...
                case Resource.Id.action_rename_Start_Deletemode:
                    Bundle bundle = new();
                    bundle.PutExtra<WlDirectory>(RenameDirectoryDialogFragment.RenameDirectoryKey, _listView.GetAdapter<WlFolderAdapter>().CurrentFolder);
